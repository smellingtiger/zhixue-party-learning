...
export async function POST(request: NextRequest) {
  try {
    const { courseName, segments } = await request.json() as {
      courseName: string;
    ...
    const segmentsText = segments.map(s =>
      `段落${s.index}：${s.content.substring(0, 300)}`
    ).join('\n');

    const systemPrompt = `你是一个专业的课程大纲设计师。任务是为课程中缺少标题的段落生成简洁、有学习指导意义的小标题。
...
[{"index": 1, "title": "..."}, {"index": 2, "title": "..."}]`;
    ...
    const userMessage = `课程标题：${courseName}

以下段落缺少标题，请根据段落内容为每个段落生成学习导向小标题：
