...
import { coldWaveKnowledgeData } from '../cold-wave-knowledge-data';
    ...
    <div className="prose prose-gray max-w-none">
      <ReactMarkdown
        ...
        components={{
          ...
          blockquote: ({ children }) => (
            <blockquote className="my-6 border-l-4 border-blue-400 bg-gradient-to-r from-blue-50 to-indigo-50 py-4 px-6 rounded-r-xl relative shadow-sm">
...
export default function ColdWaveKnowledgePage() {
  const router = useRouter();
  const course = coldWaveKnowledgeData;
  ...
  const getChapterColor = (index: number) => {
    const colors = [
      ...
      'bg-sky-500', 'bg-violet-500', 'bg-purple-500',
    ];
  ...
  if (viewMode === 'chapter') {
    return (
      <div className="h-full overflow-auto bg-gradient-to-br from-blue-50 via-indigo-50 to-white">
        <div className="container mx-auto px-4 py-6 max-w-4xl">
          ...
          <div className="bg-white rounded-xl border border-blue-100 shadow-sm mb-4 overflow-hidden">
            {!speechLoading && speechContents.length > 0 && (
              <div className="border-b border-blue-100 p-4 bg-gradient-to-br from-blue-50/50 to-indigo-50/50">
                <DigitalAvatar
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-500 text-white p-5">
  ...
  return (
    <div className="container mx-auto px-4 py-8 flex-1 overflow-y-auto">
      ...
      <div className="border-2 border-black bg-white p-6 relative mb-6" style={{ boxShadow: '4px 4px 0 0 #000' }}>
        <div className="absolute -top-3 left-4 bg-blue-600 text-white text-xs font-black px-3 py-1 border-2 border-black" style={{ boxShadow: '2px 2px 0 0 #000' }}>
          课程简介
        </div>
        <div className="flex items-center justify-between mb-4 mt-2">
          <h3 className="font-black text-2xl md:text-3xl text-black">{course.courseName}</h3>
          <div className="flex gap-2 flex-shrink-0">
            <Button size="sm" variant="outline" className={`border-2 font-bold ${isFavorited ? 'border-blue-400 bg-blue-400 text-black' : 'border-blue-400 text-blue-600 hover:bg-blue-400 hover:text-black'}`} style={{ borderRadius: '0' }} onClick={() => setIsFavorited(!isFavorited)}>
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          {[
            { label: '课程类型', value: course.courseType, color: 'bg-blue-500', icon: '📖' },
            { label: '总学时', value: `${course.totalHours}学时`, color: 'bg-blue-400', icon: '⏱' },
            { label: '难度等级', value: course.difficulty, color: 'bg-indigo-500', icon: '📊' },
            { label: '章节数', value: `${course.chapters.length}章`, color: 'bg-indigo-500', icon: '📑' },
          ].map((stat, idx) => (
            <div
              key={idx}
            ...
            <div key={chapter.id} className="p-4 border-2 border-black bg-white relative" style={{ boxShadow: '3px 3px 0 0 #000' }}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex gap-2">
                  <Button size="sm" className="bg-blue-400 text-black font-bold border-2 border-black hover:bg-blue-500" style={{ borderRadius: '0', boxShadow: '2px 2px 0 0 #000' }} onClick={() => handleEnterChapter(idx)}>
      ...
      <div className="border-2 border-black bg-gray-900 p-5 flex items-center justify-between text-white" style={{ boxShadow: '4px 4px 0 0 #000' }}>
        <div className="text-sm">
          课程：<span className="font-bold text-blue-400">{course.courseName}</span> ·
          共<span className="font-bold text-blue-400">{course.chapters.length}</span>章节 ·
          <span className="font-bold text-blue-400">{course.totalHours}</span>学时
        </div>
        <div className="flex gap-3">
          <Button size="lg" className="bg-blue-500 text-white font-bold border-2 border-black hover:bg-blue-600" style={{ borderRadius: '0', boxShadow: '3px 3px 0 0 #000' }} onClick={() => handleEnterChapter(0)}>
