...
import { forestFireKnowledgeData } from '../forest-fire-knowledge-data';
...
function RealImage({ src, alt }: { src: string; alt: string }) {
  ...
  if (error) {
    return (
      <div className="aspect-video bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-xl flex items-center justify-center my-6">
        <div className="text-center">
          <ImageIcon className="w-8 h-8 text-green-300 mx-auto mb-2" />
    ...
    <div className="prose prose-gray max-w-none">
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <div className="mb-6">
              <h1 className="text-2xl font-black text-gray-900 flex items-center gap-3">
                <span className="w-1.5 h-7 bg-gradient-to-b from-green-500 to-emerald-500 rounded-full inline-block flex-shrink-0" />
                {children}
              </h1>
              <div className="w-full h-px bg-gradient-to-r from-green-200 via-emerald-200 to-transparent mt-3" />
            </div>
          ),
          h2: ({ children }) => {
            ...
            if (titleStr.includes('参考链接')) {
              return (
                <div className="my-6 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 border-l-4 border-l-green-500 rounded-r-xl py-4 px-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center">
                      <span className="text-white text-xs">📚</span>
                    </div>
                    <span className="font-bold text-green-700 text-sm">参考链接</span>
                  </div>
                </div>
              );
            }
            return (
              <div className="mb-4 mt-7">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2 pl-4 relative">
                  <span className="absolute left-0 w-1 h-5 bg-gradient-to-b from-emerald-400 to-green-400 rounded-full" />
                  {children}
                </h2>
              </div>
            );
          ...
          strong: ({ children }) => (
            <strong className="font-bold text-green-800 bg-gradient-to-r from-green-50 to-emerald-50 px-2 py-0.5 rounded border-l-2 border-green-400">
              {children}
            </strong>
          ),
          a: ({ children, href }) => (
            <a
              ...
              className="text-green-600 underline decoration-2 underline-offset-2 hover:text-green-800 hover:decoration-green-800 hover:bg-green-50 px-1 transition-colors rounded"
            >
              {children}
            </a>
          ),
          p: ({ children }) => {
            ...
            if (content.startsWith('**参考链接**') || content.startsWith('**参考链接：')) {
              return (
                <div className="my-6 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 border-l-4 border-l-green-500 rounded-r-xl py-4 px-5 shadow-sm">
                  ...
                  <div className="text-sm text-green-600 leading-relaxed space-y-2">
                    {content
                      .replace(/^\*\*参考链接\*\*[：:]?\s*/, '')
                      .split('\n')
                      ...
                      .map((line, i) => (
                        <div key={i} className="flex items-start gap-3 group">
                          <span className="text-green-400 mt-1 group-hover:text-green-600 transition-colors">›</span>
                          <span className="group-hover:text-green-800 transition-colors">{line.trim()}</span>
                        </div>
                      ))}
                  </div>
                </div>
            ...
            if (content.startsWith('核心要点：') || content.startsWith('核心要点:')) {
              return (
                <div className="my-6 p-5 bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 border-l-4 border-l-green-500 rounded-r-xl shadow-sm">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center">
                      <Target className="h-3 w-3 text-white" />
                    </div>
                    <span className="font-bold text-green-700 text-sm">核心要点</span>
                  </div>
          ...
          ul: ({ children }) => (
            <ul className="space-y-3 mb-6 list-none pl-0 [&_li]:before:content-['•'] [&_li]:before:text-green-400 [&_li]:before:font-bold [&_li]:before:mr-3 [&_li]:before:inline-block [&_li]:before:text-lg">
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className="space-y-3 mb-6 list-decimal pl-8 marker:text-green-500 marker:font-bold marker:text-base">
              {children}
            </ol>
          ),
          blockquote: ({ children }) => (
            <blockquote className="my-6 border-l-4 border-green-400 bg-gradient-to-r from-green-50 to-emerald-50 py-4 px-6 rounded-r-xl relative shadow-sm">
              <div className="absolute top-2 right-4 text-green-200 text-5xl font-serif leading-none select-none">"</div>
          ...
          td: ({ children }) => (
            <td className="px-4 py-3 leading-relaxed text-gray-800 border border-gray-300 hover:bg-green-50 transition-colors">
              {children}
            </td>
          ),
          em: ({ children }) => (
...
export default function ForestFireKnowledgePage() {
  const router = useRouter();
  const course = forestFireKnowledgeData;
  ...
  const getChapterColor = (index: number) => {
    const colors = [
      'bg-green-500', 'bg-emerald-500', 'bg-amber-500',
    ...
    return (
      <div className="h-full overflow-auto bg-gradient-to-br from-green-50 via-emerald-50 to-white">
        <div className="container mx-auto px-4 py-6 max-w-4xl">
          ...
          <div className="bg-white rounded-xl border border-green-100 shadow-sm mb-4 overflow-hidden">
            {!speechLoading && speechContents.length > 0 && (
              <div className="border-b border-green-100 p-4 bg-gradient-to-br from-green-50/50 to-emerald-50/50">
                <DigitalAvatar
  ...
  return (
    <div className="container mx-auto px-4 py-8 flex-1 overflow-y-auto">
      ...
      <div className="border-2 border-black bg-white p-6 relative mb-6" style={{ boxShadow: '4px 4px 0 0 #000' }}>
        <div className="absolute -top-3 left-4 bg-green-600 text-white text-xs font-black px-3 py-1 border-2 border-black" style={{ boxShadow: '2px 2px 0 0 #000' }}>
          课程简介
        </div>
        ...
        <div className="flex flex-wrap gap-3 mb-5">
          {[
            { label: '课程类型', value: course.courseType, color: 'bg-green-500', icon: '📖' },
            { label: '总学时', value: `${course.totalHours}学时`, color: 'bg-amber-400', icon: '⏱' },
            { label: '难度等级', value: course.difficulty, color: 'bg-emerald-500', icon: '📊' },
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    ...
                    <span className="text-xs px-2 py-0.5 font-bold border border-black bg-green-100">
                      📑 图文课
                    </span>
                  </div>
                </div>
